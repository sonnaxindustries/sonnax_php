<?		/*
		...
		*/
		if ($_GET["new_only"] == "true" && $part_finder->product_line != 10) {//new part IF?>
			...
			<span style='color: #024368; font-size: 22px;'>New</span> <span style='color: #024368; font-size: 16px;'><?=$title?> Products</span><BR><br />
		...
		<div class="long">
			...
			if (!empty($part_finder->make_id)) {
				...
				/*
				// print unit links on the page
				echo "<div style='width:455px;'>";
				$unit_count = count($unitsBrief->displays);
				for ($i=0;$i<$unit_count;$i++) {
					echo "<div style='float: left; width: 150px;'>";
					echo "<a href='part_finder.php?" .
							"pl=" . $unitsBrief->product_line . 
							"&make=" . $unitsBrief->make .
							"&unit=" . $unitsBrief->values[$i] . 
						   "'>" . 
							$unitsBrief->displays[$i] . 
						 "</a>";
					echo "</div>";
				}
				echo "</div>";
