	<a href="speed_order.php">Speed Order Form</a>&nbsp; &nbsp; &nbsp; &nbsp;<a href="TC-PDFS/PREPACKAGED.pdf">Packaged Quantities</a><BR />
	<BR />
	<div id="scroll">
	...
	if ( !is_null($part_finder->part_stack) && !is_null($part_finder->part_stack->stack) && $part_finder->search_state ==2 ) {
		...
		while ($part = $part_stack->removePart()) {
			...
			<div class='med<?=$str_highlighted?>'><a href='part_summary.php?id=<?=$part->id?>&pl=<?=$part_finder->product_line?>'><?=$part->part_number?></a></div>
			<div class='short'>
			<?
		...
		while ($part = $part_stack->removePart()) { 					// a part object from the array of parts
			...
			for ($x=0; $x <= $makesUpperBound; $x++) {
				...
				<div class='med_short<?=$str_highlighted?>'><a href='part_summary.php?id=<?=$part->id?>&pl=<?=$part_finder->product_line?>&make=<?=$makes[$x]?>&unit='><?=$part->part_number?></a></div>
