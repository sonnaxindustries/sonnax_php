...
-->
        ...
        <p>
        <table width="285" border="0" cellpadding="0">
          <tr>
            <td colspan="2" class="style2">NEW parts since last cat<span class="style3">alog (TS</span>7):</td>
          </tr>
          <tr>
            <td width="117"><div><a href="ts-npa-2009.pdf"><strong>PDF Version</strong></a> </div></td>
            <td width="162"><a href="part_finder.php?pl=3&amp;new_only=true"><strong>Interactive (in blue)</strong></a></a></td>
          </tr>
        </table>
<div></div> 
</p>
		...
		// This is a TS search. If a make and unit exist then try to find the mirror unit in HPT
            ...
            if ( !empty( $_GET["make"] ) && !empty( $GLOBALS['unit_name'] ) ) {
                ...
                if ( $tsMirrorUnitId ) {?>
                    <a href="part_finder.php?unit=<?php echo $tsMirrorUnitId;?>&make=<?php echo urlencode($_GET["make"]);?>&pl=1">View High Performance Transmission Unit <?php echo $GLOBALS['make_name'];?> <?php echo $GLOBALS['unit_name'];?></a>
                <?php }
            }
		?>
<div id='scroll'>
