		...
		<form name='form_driveline_series' id='form_driveline_series' method='get' action='part_finder.php' class='form'>
			<?
			require_once $part_finder_admin_path_adjustment."includes/classes/clsDrivelines.php";
			$tube_diameter		= new Drivelines();
			FormObjects::selectBoxForDrivelineSeries("driveline_series", $tube_diameter->displays, $tube_diameter->values, stripslashes($_GET["driveline_series"]), "dropdown", "driveline_series"); ?>
			&nbsp;&nbsp;
		...
		<div class='long'>
			...
			<?
			require_once $part_finder_admin_path_adjustment."includes/classes/clsTubeDiameterDriveline.php";
			$tube_diameter		= new TubeDiameterDriveline();
			FormObjects::selectBoxForTubeDiameter("tube_diameter", $tube_diameter->displays, $tube_diameter->values, stripslashes($_GET["tube_diameter"]), "dropdown", "tube_diameter"); ?>
			<!-- <input type='submit' name='go' value='GO' class='submit' style='width:34px;'>  -->
			<input type=hidden name=pl value="<?=$part_finder->product_line?>">
		</div>
		</form>
