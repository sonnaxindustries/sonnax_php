		...
		<div class="long" style='width:340px;'>
			...
			<input type='text' name='oem_pn' class='field' value='<?=htmlentities(stripslashes($get["oem_pn"]),ENT_QUOTES)?>'>&nbsp;&nbsp;
			<input type='submit' name='go' value='GO' class='submit' style='width:34px;'>
			<input type=hidden name=pl value="<?=$part_finder->product_line?>">
		</div>
		</form>
		...
		<div class="long">
			<div class='label'><strong>Unit:</strong></div>
				<? FormObjects::selectBox("unit", $unitsBrief->displays, $unitsBrief->values, htmlentities(stripslashes($get["unit"]),ENT_QUOTES), "dropdown", "unit"); ?>
				&nbsp;&nbsp;
			<input type='submit' name='go' value='GO' class='submit' style='width:34px;'>
		</div>
		<input type=hidden name=pl value="<?=$part_finder->product_line?>">
