				...
				<!-- REGULAR BAR -->		
					...
					if (is_array($arr_data)) {
						...
						for ($x=0;$x<=$int_num_data;$x++) {?>
							<div class='longest'><a href='part_summary.php?id=<?=$arr_data["parts.id"][$x]?>&ucid=<?=$arr_data["unit_components.id"][$x]?>&pl=<?=$part_finder->product_line?>'><?=$arr_data["parts.part_number"][$x]?></a></div>
