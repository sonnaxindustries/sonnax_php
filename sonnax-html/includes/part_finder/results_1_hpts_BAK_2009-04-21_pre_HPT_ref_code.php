		...
		// This is an HPT search. If a make and unti exist then try to find the mirror unit in TS
            ...
            if ( !empty( $_GET["make"] ) && !empty( $GLOBALS['unit_name'] ) ) {
                ...
                if ( $tsMirrorUnitId ) {?>
                    <a href="part_finder.php?unit=<?php echo $tsMirrorUnitId;?>&make=<?php echo urlencode($_GET["make"]);?>&pl=3">View Transmission Specialties Unit <?php echo $GLOBALS['make_name'];?> <?php echo $GLOBALS['unit_name'];?></a>
                <?php }
            }
		?>
