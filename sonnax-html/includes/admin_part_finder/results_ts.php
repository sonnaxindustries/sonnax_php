	...
	if ( !is_null($part_finder->part_stack) && !is_null($part_finder->part_stack->stack) && $part_finder->search_state ==2 ) {
		...
		while ($part = $part_stack->removePart()) { 			// a part object from the array of parts
			...
			<div class='med_long2' style='font-weight:bold;'>
				...
				<a href="system_editor_exe.php?action=remove_part_from_unit&pl=<?=htmlentities($_GET['pl'])?>&unit=<?=htmlentities($_GET['unit'])?>&part_id=<?=$part->id?>&rd=<?=urlencode($rd)?>" onclick="return fVerifyAgreementUnitPart('<?=str_replace("'","",$part->part_number)?>')">Remove Part From Unit</a>
				<? }?>
			</div>
			<div class="line"></div>
			<?
		...
		while ($part = $part_stack->removePart()) { 					// a part object from the array of parts
			...
			for ($x=0; $x <= $makesUpperBound; $x++) {
				...
				<div class='med_long2' style='font-weight:bold;'>
					<? if (strlen($_GET['pl']) > 0 && strlen($_GET['unit']) >0) {?>
					<a href="system_editor_exe.php?action=remove_part_from_unit&pl=<?=htmlentities($_GET['pl'])?>&unit=<?=htmlentities($_GET['unit'])?>&part_id=<?=$part->id?>&rd=<?=urlencode($rd)?>">Remove Part From Unit</a>
					<? }?>
				</div>
				<div class="line"></div>
				<?
