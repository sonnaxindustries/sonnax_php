...
class OrderStack  {
	...
	public function addPartLookUp ($part_id, $product_line, $quantity, $shopping_cart_id, $description) {
		...
		// ??? this serves no purpose; this code should just be in addPart above
		
		/*
		//all of these if/elseif blocks are identical
		...
		*/
		...
		if ($product_line == 1 || $product_line == 2 || $product_line == 3 || $product_line == 7 || $product_line == 10) {
			$part = new Part($part_id);
			$part->shopping_cart_quantity = $quantity;
			$part->shopping_cart_id = $shopping_cart_id;
			$part->description = $description;
