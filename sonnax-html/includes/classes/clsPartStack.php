...
class PartStack  {
	...
	public function sortByRefNumber () {  // woohoo its a bubble sort!
		usort($this->stack, array("Part", "compare_parts_tc"));
		//natcasesort($this->stack);//posible solution
		/*
		$upperBound = count($this->stack) - 1;
		for ($x = 0; $x <= $upperBound; $x++) {
			for ($y = 0; $y <= $upperBound; $y++) {
				//strnatcmp
				//if ( strcmp($this->stack[$x]->ref_number, $this->stack[$y]->ref_number) > 0 ) {//not handling letters and number correctly together
				if ($this->stack[$x]->ref_number > $this->stack[$y]->ref_number) {//not handling letters and number correctly together
					$hold = $this->stack[$x];
	...
	public function sortByRefCodeHpt () {  
		usort($this->stack, array("Part", "compare_parts_hpt_ref_code"));
	}
