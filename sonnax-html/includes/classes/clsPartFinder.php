...
class PartFinder {
		...
		switch ($this->search_state) {
			...
			case 2:				
				...
				if (!empty($unit_id)) {
					...
					if ( $this->product_line == 1 ) {
					    // Sort HPT parts by their ref_code_sort integer field
					    $no_return = $this->part_stack->sortByRefCodeHpt();
					} else {
					    $no_return = $this->part_stack->sortByPartNumber();
					}
				} else {
	...
	public function partStackSetupTCLine() {
		...
		switch ($this->search_state) {
			...
			case 1: 
				// perform part search and set part_list
				$this->part_stack = new PartStack();
				if ($this->part_number_search == "none") {
					$sql = "
						SELECT
							`id`
						FROM `parts` 
			...
			case 2:
				...
				if (!empty($unit_id)) {
					...
					$no_return = $this->part_stack->sortByRefNumber();
				} else {
					$this->search_state = 0;
					$no_return = $this->setErrorMessage("");
				}
				...
				*/
				$sql = "
					SELECT
						...
						`unit_components`.`pts_series`, `unit_components`.`driveline_series`, `unit_components`.`id` 
						FROM `parts` 
							LEFT JOIN `unit_components` ON `parts`.`id` = `unit_components`.`assembly_or_part_id` 
						WHERE 
							`parts`.`part_number` LIKE '".mysql_real_escape_string($this->part_number_search)."%' 
							AND	`parts`.`product_line` = '".mysql_real_escape_string($this->product_line)."' 
							AND (
								`unit_components`.`component_type` IS NULL
								OR `unit_components`.`component_type` = 0
								)
						ORDER BY `parts`.`part_number`, `unit_components`.`driveline_series`,`unit_components`.`steel_driveshaft_tube_od`";
				break;
			case 9:
				...
				$sql = "
					SELECT
						...
						`unit_components`.`pts_series`, `unit_components`.`driveline_series`, `unit_components`.`id` 
						FROM `unit_components` 
							INNER JOIN `parts` ON `unit_components`.`assembly_or_part_id` = `parts`.`id` 
						WHERE 
	...
	private function determineSearchState () {
		...
		//new item override
			if ($_GET["new_only"] == "true" && ($this->product_line == 1 /* HPT */ || $this->product_line == 2 /* TC */ || $this->product_line == 3 /* TS */ || $this->product_line == 7 /* Allison */ || $this->product_line == 12 || $this->product_line == 13 || $this->product_line == 14)) {
				$this->search_state = 1;
				return;
			}
		...
		if ( $this->product_line == 8 && !empty($this->part_number_search) ) {
			$this->search_state = 12;
			return;
		}
	...
	//proptype of the new searhc function
	public function newPartSearchTempFunction () {
		$sql = "
			SELECT
				`parts`.*,`unit_components`.*,`units`.*  
				FROM (`unit_components` 
					LEFT JOIN `parts` 
						ON `unit_components`.`assembly_or_part_id` = `parts`.`id`) 
					LEFT JOIN `units` 
						ON `unit_components`.`unit_id` = `units`.`id`
				WHERE 
					`parts`.`part_number` LIKE '".mysql_real_escape_string($this->part_number_search)."%' 
					AND (
						`unit_components`.`component_type` IS NULL
	...
	public function returnArray2dAssociative_TableDotField_Ord () {
