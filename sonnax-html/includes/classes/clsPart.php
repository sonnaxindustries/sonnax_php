...
// Fields ///////////////////////////////////////////////////////////////
	...
	// Used for labeling and sorting HTP parts on the part finder
	public $ref_code;
	public $ref_code_sort;
...
// Methods //////////////////////////////////////////////////////////////
	function __construct ($id, $ref_number = 0, $tc_description="", $debugOutput = false) 
	{
		...
		$this->loadFields($id,$debugOutput);
	}
	...
	/**
	 * convert the product line id into it's string name
	 *
	 * @return string
	 */
	public function getProductLineName() {
		if ( is_numeric($this->product_line) ) {
			$pl = new ProductLine($this->product_line);
			return $pl->name;
		} else {
			return "";
		}
	...
	public function loadFields ($id,$debugOutput = false)
	{
		if (is_numeric($id)) {
			...
			if (is_array($partData)) {
				...
				$this->ref_code = $partData["parts.ref_code"][0];
				$this->ref_code_sort = $partData["parts.ref_code_sort"][0];
	...
	/**
	 * Compare two tc part objects for use with a usort in a PartStack object
	 * Comparison is based on the integer field ref_code_sort
	 *
	 * @param Part $part_1  - part object
	 * @param Part $part_2  - part object
	 * @return sort order indicator
	 */
	public function compare_parts_hpt_ref_code( $part_1, $part_2 )
	{
	    // If the IF block uses -1 and the ELSEIF uses +1 they sort low to high
	    // If the IF block uses +1 and the ELSEIF uses -1 they sort high to low
	    if ( $part_1->ref_code_sort > $part_2->ref_code_sort) {
	        // Greater than
	        return -1;
	    } elseif ( $part_1->ref_code_sort < $part_2->ref_code_sort) {
	        // Less than
	        return 1;
	    } else {
	        // Equal
	        // Attempt a secondary sort
	        if ( $part_1->part_number > $part_2->part_number) {
	            // Greater than
	            return -1;
	        } elseif ( $part_1->part_number < $part_2->part_number) {
	            // Less than
	            return 1;
	        } else {
	            return 0;
