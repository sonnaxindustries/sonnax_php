...
// Methods //////////////////////////////////////////////////////////////
	...
	public function unitComponents ($unit_id) {		
		...
		*/
		...
		$sqlTemp = "
			...
			WHERE unit_components.unit_id = '".mysql_real_escape_string($unit_id)."'";
		if ($_GET["show_only_sc"] == "true" && $_GET["show_only_tt"] == "true") {
			$sqlTemp .= " AND parts.product_line_from_ts_file IN ('TT','SC')";
		} elseif ($_GET["show_only_sc"] == "true") {
			$sqlTemp .= " AND parts.product_line_from_ts_file = 'SC'";
		} elseif ($_GET["show_only_tt"] == "true") {
	...
	public function assemblyParts ($assembly_id) {
		...
		//NOTES: 
			...
			//I believe that the display_order field in the units_components table IS being used but possibly only for assemblyies and not parts.
