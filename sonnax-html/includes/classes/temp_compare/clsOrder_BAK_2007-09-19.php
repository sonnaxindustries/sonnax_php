...
class Order {
	...
	protected function testOrderNumber($order_number) {
		$sql = "SELECT id, TC from `order` WHERE id = {$order_number}";
	...
	protected function buildOrderStack () {
		...
		$sql = "DELETE FROM order_parts WHERE quantity = 0";
		$this->dataconn->f_ExecuteSql($sql);
		
		//get the rest of the parts for this order from the database
		$sql = "SELECT `order_parts`.`id`, `order_parts`.`part_id`, `order_parts`.`quantity`, `order_parts`.`product_line`, `order_parts`.`description` 
				FROM order_parts
	...
	public function removePart ($id) {
		$sql = "DELETE FROM order_parts WHERE id = {$id}";
		$this->dataconn->f_ExecuteSql($sql);
	...
	public function updatePart ($id, $quantity) {
		if (is_numeric($id) && is_numeric($quantity)) {
			$sql = "UPDATE order_parts SET quantity = {$quantity} WHERE id = {$id}";
			$this->dataconn->f_ExecuteSql($sql);
			$this->buildOrderStack();
		}
	}
	...
	public function addPart ($component_id, $part_quantity, $product_line, $component=0) {
		...
		if (is_numeric($component_id) && is_numeric($part_quantity) && is_numeric($product_line) && is_numeric($component)) {
			...
			$sql = "INSERT INTO order_parts (order_id, part_id, quantity, product_line, description) VALUES ({$this->order_id},{$part_id},{$part_quantity},{$product_line},'{$description}')";
			$this->dataconn->f_ExecuteSql($sql);
			$this->buildOrderStack();
		}
	}
