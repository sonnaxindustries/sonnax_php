...
// Methods //////////////////////////////////////////////////////////////
	...
	{
		if (is_numeric($id)) {
			$sql = "SELECT * FROM parts WHERE id = '".mysql_real_escape_string($id)."'";
	...
	{
		$sql = "DELETE FROM parts WHERE id = '".mysql_real_escape_string($this->id)."'";
		...
		*/
		...
		$sql = "
			...
			WHERE component_type = 0 
				AND assembly_or_part_id = '".mysql_real_escape_string($this->id)."'";
		//echo $sql . "<BR>\n";
		...
		$sql = "
			SELECT assembly_id 
			FROM assembly_parts 
			WHERE part_id = '".mysql_real_escape_string($this->id)."'";
		//echo $sql . "<BR>\n";
			...
			for ($x=0;$x<=$int_num_assemblies;$x++) {
				...
				$sql = "
					...
					WHERE component_type = 1 
						AND assembly_or_part_id = '".mysql_real_escape_string($assemblyPartData["assembly_parts.assembly_id"][$x])."'";
				//echo $sql . "<BR>\n";
				$unitData = $this->dataconn->f_ReturnArrayAssoc_TF($sql);
				if (is_array($unitData)) {
					$int_num_units = count($unitData["unit_components.unit_id"])-1;
		...
		if (strlen($make_id)>0) {
			...
			$sql = "
				SELECT unit_id 
				FROM unit_makes 
				WHERE make_id = '".mysql_real_escape_string($make_id)."'";
			//echo $sql . "<BR>\n";
	...
	/**
	 * Compare two part objects for use with a usort in a PartStack object
	 *
