...
Class DataConn {
	...
	public function __construct($database = "sonnax") {
		...
		//$this->link = mysql_connect("db.secure", "sonnax", "auto99parts"); // db2.vtweb.com / XbD.Srjf
		//$this->link = mysql_connect("localhost", "sonnax", "XbD.Srjf"); // db2.vtweb.com / XbD.Srjf
		...
		if ( !$success ) {
		    die ("Could not select database '" . $database . "' (#1)");
		}
	...
	function chooseDatabase($database) {
	    $success = mysql_select_db ($database);
		if ( !$success ) {
		    die ("Could not select database '" . $database . "' (#2)");
		}
	}
	...
	public function f_ReturnArrayAssoc_TF($sql) {
		if ($this->debug==true) {echo "\n<!-- sqldebug f_ReturnArrayAssoc_TF\n".$sql." -->\n";}
