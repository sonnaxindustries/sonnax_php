...
class Order {
	...
	protected function testOrderNumber($order_number) {
		$sql = "
			SELECT id, TC 
			FROM `order` 
			WHERE id = '".mysql_real_escape_string($order_number)."'/* Order->testOrderNumber */";
	...
	protected function buildOrderStack () {
		...
		$sql = "
			...
			FROM order_parts
			WHERE order_id = '".mysql_real_escape_string($this->order_id)."'/* Order->buildOrderStack */";
	...
	public function removePart ($id) {
		$sql = "DELETE FROM order_parts WHERE id = '".mysql_real_escape_string($id)."'/* Order->removePart */";
		$this->dataconn->f_ExecuteSql($sql);
	...
	public function updatePart ($id, $quantity) {
		if (is_numeric($id) && is_numeric($quantity)) {
			$sql = "
				...
				WHERE id = '".mysql_real_escape_string($id)."'/* Order->updatePart */";
			$this->dataconn->f_ExecuteSql($sql);
			$this->buildOrderStack();
		}
	}
		...
		if (is_numeric($component_id) && is_numeric($part_quantity) && is_numeric($product_line) && is_numeric($component)) {
			...
			$sql = "
				INSERT INTO order_parts (
					...
					'".mysql_real_escape_string($part_quantity)."',
