...
if (strlen($_GET["name"]) < 1 || strlen($_GET["company"]) < 1 ||
	...
	strlen($_GET["country"]) < 1 || strlen($_GET["email"]) < 1 ||
	strlen($_GET["coreapplications"]) < 1 || strlen($_GET["numbercores"]) < 1 ||
	strlen($_GET["type"]) < 1) {
	header("Location: solenoids.php?".$_SERVER["QUERY_STRING"]."#missing_info");
}

if (!check_email_address($_GET["email"])) {
	header("Location: solenoids.php?".$_SERVER["QUERY_STRING"]."&email_problem=true#missing_info");
}

$strSubject = "solenoids";
$strFrom = "SonnaxWebsite";
$strTo = "cores@sonnax.com,nate@theklaibers.com";
...
$strBodyMain = "
	...
	<TR><TD>Approximate Number of Cores</TD><TD>".$_GET["type"]."</TD></TR>\n
	<TR><TD>Core Applications</TD><TD>".$_GET["type"]."</TD></TR>\n
	<TR><TD>&nbsp;</TD><TD>&nbsp;</TD></TR>\n
	<TR><TD valign=top>Solenoids</TD><TD>".$solenoids."</TD></TR>\n";

mail($strTo, $strSubject, ($strBodyHeader . $strBodyMain . $strBodyFooter),$strHeaders);


header("Location: thanks_solenoid.php");
