...
<div id="container">
	...
	<div id="main">
		<div class="content">
			...
			if (!is_array($arr_component_data) && $component_id > 0) {?>
				Invalid Component (id <?=strip_tags($component_id)?>)<BR>
			<? } else {?>
				...
				<? if ($component_id == 0) {
					//adding a new component to this PTS part?>
					<div class='label'>Part ID:&nbsp;<?=$part_id?></div><BR>
					<div class='label'>Component ID: New</div><BR>
					<div class='label'>Component Type: Part</div><BR>
				<? } else {//edit existing component?>
					<div class='label'>Part ID:&nbsp;<?=$arr_component_data["unit_components.assembly_or_part_id"][0]?></div><BR>
				...
				<? }?>
					...
					<input type='hidden' name='code_on_ref_figure' value="<?=htmlentities($arr_component_data["unit_components.code_on_ref_figure"][0],ENT_COMPAT)?>">
					<input type='hidden' name='indent' value="<?=htmlentities($arr_component_data["unit_components.indent"][0],ENT_COMPAT)?>">
					<div class="cleaner"></div>
