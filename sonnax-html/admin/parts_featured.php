...
//S-----Get input-----
	$strTableName = "`parts_featured`";
	$strIdField = "`id`";
	$strDisplayField = "`product_line_id`,`part_id`";
	$strReturnFieldNames = "*";
	$strOrderBy = "`product_line_id`,`part_id`";
	
	$strLookupField_Names = "product_line_id|part_id";//foreign key fields in the return list that need to be looked up
	$strLookupField_Tables = "product_lines|parts";//table that the lookup field is being looked up in
	$strLookupField_Fields = "id,name|id,part_number";//ex: 'id,name' where id represents the foreign key and name represents the lookup value
	$strLookupField_OrderBys = "name|part_number";//field the lookup results are ordered by
...
?>
<center><a href='table_edit.php?t=<?=rawurlencode($strTableName)?>&if=<?=rawurlencode($strIdField)?>&df=<?=rawurlencode($strDisplayField)?>&rf=<?=rawurlencode($strReturnFieldNames)?>&ob=<?=rawurlencode($strOrderBy)?>&wh=<?=rawurlencode($strWhere)?><?=$strTableEditParams?>&id=0'>Add New Featured Part</a></center>
...
for($q = $start_position; $q <= $end_position; $q++) { //0 to intNumRecords
	...
	<tr bgcolor="#<?=$strBGcolorTemp?>">
		...
		<?for($col = 1; $col <= $intNumColumns; $col++) {
			$pos1 = strpos($arrTableFieldStructure[$col][0], "product_line");
			$pos2 = strpos($arrTableFieldStructure[$col][0], "part_id");
			if ($pos1 !== false && $pos1 >= 0) { // note: three equal signs?>
			<td align=left><font size=3>&nbsp;<?=getProductLineName($arrTableData[$col][$q])?></font></td>
			<? } elseif ($pos2 !== false && $pos2 >= 0) { // note: three equal signs?>
			<td align=left><font size=3>&nbsp;<?=getPartNumber($arrTableData[$col][$q])?></font></td>
			<?} else {//do a lookup on the product_line foield?>
			<td align=left>
			<font size=3>&nbsp;<?=$arrTableData[$col][$q]?></font></td>
			<?}
		}?>
