...
//S-----Get input-----
	...
	$strRedirect = $_GET["rdir"];
		if (strlen($strRedirect) <= 0) {
			$strRedirect = $HTTP_REFERER; //$REQUEST_URI; //getenv("HTTP_REFERER"); //ASP equiv Request.ServerVariables("HTTP_REFERER");
		}
	//E-----Get input-----
	...
	if ($strFieldNameLcaseTemp == $strIdFieldTemp) {
		...
		if ($strID == "0") { /*they are adding so we show nothing*/?>
			<td width=400>
				...
				<input type=hidden name='field<?=$q?>_a' value='<?=$arrTableFieldStructure[$q][0]?>'>
				<input type=hidden name='field<?=$q?>_b' value='<?=$arrTableFieldStructure[$q][1]?>'>
				<input type=hidden name='field<?=$q?>_c' value='<?=$arrTableFieldStructure[$q][2]?>'>
		...
		<?} else { /*display the field as text so as to make it non-editable*/?>
			<td width=400>
				...
				<input type=hidden name='field<?=$q?>_c' value='<?=$arrTableFieldStructure[$q][2]?>'>
				<input type=hidden name='field<?=$q?>_d' value='<?=$strFieldData?>'>
				<input type=hidden name='field<?=$q?>_e' value="">
			</td>
		<?}
		...
		switch ($arrTableFieldStructure[$q][1]) {
			case "string": /* char, varchar */
				...
				<td width=400>
					...
					<input type=hidden name='field<?=$q?>_c' value='<?=$arrTableFieldStructure[$q][3]?>'>
					...
					<?} elseif ($strTableName == "parts" && $strFieldNameLcaseTemp == "product_line_from_ts_file") {
						...
						<input type=text name='field<?=$q?>_d' maxlength='<?=$arrTableFieldStructure[$q][2]?>' value='<?=$strFieldData?>' size=50>
					<?}?>
					<input type=hidden name='field<?=$q?>_e' value="'">
				</td>
				<?
			...
			case "int": /* tinyint, smallint, mediumint, int, & bigint*/?>
				...
				//?? case 3	could be done more elegantly but this works
						...
						if ($bNonEditable == True) {
							...
							//echo "65443<BR>";?>
							<input type=text name='field<?=$q?>_d' maxlength='<?=$arrTableFieldStructure[$q][2]?>' value='<?=$strFieldData?>' size="10"> integer
							<?
						} else {
			...
			case "date": /* date, datetime, timestamp, time, year (artificially broadened above) */?>
				<td width=400>
					...
					<?if ($bNonEditable == True) {?>
						...
						<input type=hidden name='field<?=$q?>_d' value='<?=$strFieldData?>'>
					<?} else {?>
						<input type=text name='field<?=$q?>_d' maxlength="22" value='<?=$strFieldData?>' size="50"><BR>(YYYY-MM-DD hh:mm:ss) Use 24hr time format
					<?}?>
					<input type=hidden name='field<?=$q?>_e' value="'">
				</td>
				<?break;
			case "real": /* float, double, decimal*/?>
				<td width=400>
					...
					<?if ($bNonEditable == True) {?>
						...
						<input type=hidden name='field<?=$q?>_d' value='<?=$strFieldData?>'>
					<?} else {?>
						<input type=text name='field<?=$q?>_d' maxlength="20" value='<?=$strFieldData?>' size="10"> real number
					<?}?>
					<input type=hidden name='field<?=$q?>_e' value="">
				</td>
				<?break;
...
<tr>
	<td colspan=2 align=center>
		...
		?>
		<BR>
		<input type=submit value='<?=$strButtonValue?>' id=submit1 name=submit1>
	</td>
</tr>
</table>
</FORM>
