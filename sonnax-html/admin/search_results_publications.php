...
//S-----Get input-----
	$search_for = $_GET["search_for"];
		...
		//	$strMessage = "Search input is required";
		//	f_OutputErrorNow($strMessage);
		//}
	$strTableName        = "`publication_titles`";
	$strIdField          = "`id`";
	$strDisplayField     = "`title`";
	$strReturnFieldNames = "*";
	$strOrderBy          = "`title`";
	$strSearcField     = "`title`";
	
	//$strLookupField_Names = "'product_line'";//foreign key fields in the return list that need to be looked up
	//$strLookupField_Tables = "'product_lines'";//table that the lookup field is being looked up in
	//$strLookupField_Fields = "'id,name'";//ex: 'id,name' where id represents the foreign key and name represents the lookup value
	//$strLookupField_OrderBys = "'name'";//field the lookup results are ordered by
	...
	//result paging variables
		$num_results_to_show = 50; //should always be even (odd numbers cause the row bgcolors to be off by one on some multiple page tables)
	...
	$sqlTemp = "SELECT " . mysql_real_escape_string($strDisplayField_TEMP) . " " .
		"FROM " . mysql_real_escape_string($strTableName) . " " .
		"WHERE " . mysql_real_escape_string($strSearcField) . " LIKE '" . mysql_real_escape_string($search_for) . "%' " .
		"ORDER BY " . mysql_real_escape_string($strOrderBy) . ";";
	//echo $sqlTemp . "<BR>";
	//exit;
...
for($q = $start_position; $q <= $end_position; $q++) { //0 to intNumRecords
	...
	<tr bgcolor="#<?=$strBGcolorTemp?>">
		...
		<?for($col = 1; $col <= $intNumColumns; $col++) {
			$pos = strpos($arrTableFieldStructure[$col][0], "product_line");
			if ($pos === false) { // note: three equal signs?>
			<td align=left><font size=3>&nbsp;<?=$arrTableData[$col][$q]?></font></td>
			<?} else {//do a lookup on the product_line foield?>
			<td align=left><font size=3>&nbsp;<?=getProductLineName($arrTableData[$col][$q])?></font></td>
			<?}
		}?>
...
function f_OutputPagingSystem($strTableName,$strIdField,$strDisplayField,$strReturnFieldNames,$strOrderBy,$strRedirect,$start_position,$end_position,$intNumRecords,$page_number,$total_pages,$strLookupField_Names,$strLookupField_Tables,$strLookupField_Fields,$strLookupField_OrderBys,$strNonEditableFields,$search_for) {
	//print the result totals and the result page navigation
		$strBaseAction = "search_results_publications.php?t=" . rawurlencode($strTableName) . "&if=" . rawurlencode($strIdField) . "&df=" . rawurlencode($strDisplayField) . "&rf=" . rawurlencode($strReturnFieldNames) . "&ob=" . rawurlencode($strOrderBy) . "&rdir=" . rawurlencode($strRedirect) . "&search_for=" . rawurlencode($search_for);
		...
		echo "<table WIDTH='100%' align=center cellspacing=2 cellpadding=2 bgcolor=#DDDDDD><tr>" .
			...
			"<FORM name=pageset method=GET action='search_results_publications.php'>\n" .
