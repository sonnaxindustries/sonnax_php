...
$assembly_name =		getAssemblyName($assembly_id);
$assembly_part_id = 	$_GET["assembly_part_id"];
$code_on_ref_figure = 	getCodeOnRefFigureFromAssemblyPartId($assembly_part_id);
$part_id = 				getPartIdFromAssemblyPartId($assembly_part_id);
$part_number = 			getPartNumber($part_id);
...
<div id="container">
	...
	<div id="main">
		<div class="content">
			...
			<?
			if (!is_numeric($assembly_id) || $assembly_name == -1) {?>
				Invalid Assembly (id: <?=assembly_id?>)<BR>
			<? } else {?>

			<form name='form1' id='form1' method='get' action='system_editor_exe.php' class='form' style="padding-left:50px;">
			<div class="long">
				<div class='label'>Assembly Name: <?=strip_tags($assembly_name)?></div><BR>
				<div class='label'>Part Number: <?=strip_tags($part_number)?></div><BR>
				<BR>
				<div class='label'>Code on Reference Figure:</div>
				<input type='text' name='code_on_ref_figure' class='field' value="<?=htmlentities($code_on_ref_figure,ENT_COMPAT)?>"><BR>
				<div class="cleaner"></div>
				<input type='submit' name='go' value='GO' class='submit' style='width:34px;'>
				<input type="hidden" name="action" value="edit_assembly_part_ref_code">
				<input type="hidden" name="rd" value="<?=$rd?>">
				<input type="hidden" name="assembly_part_id" value="<?=$assembly_part_id?>">
			</div>
			</form>
			<div class="cleaner"></div>
			<? }?>
